...
func (k *kafkaReceiver) Receive(ctx context.Context, handler event.Handler) error {
	go func() {
		for {
			...
			if err != nil {
				slog.Error("kafka fetch message", "err", err)
				break
			}
			...
			if err != nil {
				slog.Error("message handling exception", "err", err)
			}
			if err := k.reader.CommitMessages(context.Background(), m); err != nil {
				slog.Error("failed to commit messages", "err", err)
			}
		}
	}()
	return nil
